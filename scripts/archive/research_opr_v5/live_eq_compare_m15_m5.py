...
def compute_f2_pre_fill(df_ny, trigger_ts, fill_ts, direction,
                          opr_high, opr_low, atr_daily, exclude_fill_bar=True):
...
def main():
    ...
    for ticker in SELECTED_TICKERS:
        ...
        trades = v51.run_backtest(df_15m, ticker, tf=tf, params=None,
                                   topstep_guard=False)
        ...
        for _, row in filled.iterrows():
            ...
            f2_m15 = compute_f2_pre_fill(df_15m_ny, trigger_ts, fill_ts,
                                          row["dir"], opr_h, opr_l, atr_d)
            # F2 M5 pre-fill (exclut seulement les 5 dernières minutes)
            f2_m5  = compute_f2_pre_fill(df_m5_ny, trigger_ts, fill_ts,
                                          row["dir"], opr_h, opr_l, atr_d)
        ...
        print(f"    M15 fidélité : {n - n_rejected_m15}/{n}  =  {fid_m15:.1f} %   (rejected {n_rejected_m15})")
        print(f"    M5  fidélité : {n - n_rejected_m5}/{n}  =  {fid_m5:.1f} %   (rejected {n_rejected_m5})")
