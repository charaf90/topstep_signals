...
def main():
    parser = argparse.ArgumentParser(description="Validation M1Buffer (WS) vs REST get_bars")
    parser.add_argument(
        "--duration", type=int, default=5, help="Durée d'écoute en minutes (défaut 5)"
    )
    parser.add_argument("--symbol", type=str, default="MNQ", help="Symbol à valider (défaut MNQ)")
    ...
    while time.monotonic() < deadline:
        ...
        if n_drain % 30 == 0:
            health = buf.health()
            print(
                f"  [t={n_drain}s] trades_consumed={health['trades_consumed']}  "
    ...
    print(f"\n{'─' * 80}")
    print("COMPARAISON BAR PAR BAR (WS vs REST)")
    print(f"{'─' * 80}")
    ...
    for ws_bar in ws_bars:
        ...
        deltas.append(d)
        common_count += 1
        if d["high_within_1_tick"]:
            high_match_1tick += 1
        if d["low_within_1_tick"]:
            low_match_1tick += 1
        if (
            d["delta_open"] == 0
            and d["delta_high"] == 0
        ...
        rest_str = (
            f"O{rest['open']:.2f} H{rest['high']:.2f} L{rest['low']:.2f} C{rest['close']:.2f}"
        )
        print(
            f"{ts.strftime('%Y-%m-%d %H:%M'):<22} {ws_str:<36} {rest_str:<36} "
    ...
    rest_only = [ts for ts in rest_by_ts if ts not in ws_ts_set and start_utc <= ts <= end_utc]
    ...
    if common_count > 0:
        print(
            f"  High match ≤1 tick : {high_match_1tick}/{common_count} "
            f"({100 * high_match_1tick / common_count:.1f}%)"
        )
        print(
            f"  Low  match ≤1 tick : {low_match_1tick}/{common_count} "
            f"({100 * low_match_1tick / common_count:.1f}%)"
        )
        print(
            f"  OHLC perfect match : {perfect_match}/{common_count} "
