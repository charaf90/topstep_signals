...
def metrics(trades: pd.DataFrame) -> dict:
    if len(trades) == 0:
        return dict(n=0, pf=float("nan"), pnl=0.0, dd=0.0, sharpe=float("nan"),
                    wr=float("nan"))
...
def slice_oos(trades: pd.DataFrame) -> pd.DataFrame:
    return trades[(trades["pending_time"] >= OOS_START) &
                  (trades["pending_time"] <= OOS_END)].copy()
...
def main():
    ...
    lines.append("- **fib-v3** : configuration LIVE actuelle "
                 "(`FIB_LEVEL_PER_TICKER=0.382` + filtres trigger calibrés).")
    lines.append("- **fib-v4** : `FIB_V4_LEVEL_PER_TICKER` + invalidation pivot "
                 "break + wick excess data-driven.\n")
    lines.append("Frictions appliquées : slippage `config.SLIPPAGE_TICKS_PER_TICKER` "
                 "(2 côtés) + commission RT $1.40.\n")

    portfolio_v3, portfolio_v4 = [], []

    lines.append("\n## Comparaison TOTAL (toute l'historique sept 2024 → mai 2026)\n")
    lines.append("| Ticker | v3 n | v3 PF | v3 P&L | v3 DD | v4 n | v4 PF | v4 P&L | v4 DD | Sélect. |")
    lines.append("|---|---|---|---|---|---|---|---|---|---|")
    ...
    lines.append(f"| **PORTEFEUILLE** | **{mp3['n']}** | **{mp3['pf']:.2f}** "
    ...
    lines.append("|---|---|---|---|")
    for ticker, t3, t4 in zip([c[0] for c in PROD_CELLS],
                                portfolio_v3_oos, portfolio_v4_oos):
        d3 = daily_pnl(t3)
        d4 = daily_pnl(t4)
    ...
    lines.append("Si la sélectivité est intelligente, les trades supprimés doivent "
                 "avoir un P&L cumulé majoritairement négatif (= on a éliminé les pertes).\n")
    lines.append("| Ticker | Trades v3 OOS | Refusés par v4 | P&L cumulé refusés | "
                 "PF des refusés |")
    lines.append("|---|---|---|---|---|")
    for ticker, t3, t4 in zip([c[0] for c in PROD_CELLS],
                                portfolio_v3_oos, portfolio_v4_oos):
    ...
    for ax, (ticker, _), t3, t4 in zip(axes[0], PROD_CELLS,
                                          portfolio_v3_oos, portfolio_v4_oos):
        if len(t3):
            t3 = t3.sort_values("pending_time")
            ax.plot(t3["pending_time"], t3["pnl"].cumsum(),
                    label=f"v3 (n={len(t3)})", color="steelblue")
        if len(t4):
            t4 = t4.sort_values("pending_time")
            ax.plot(t4["pending_time"], t4["pnl"].cumsum(),
                    label=f"v4 (n={len(t4)})", color="darkorange")
