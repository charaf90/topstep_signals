...
def main() -> int:
    ...
    rows.append("")
    rows.append(
        "Cible : ∈ [0.7, 0.95] pour valider un edge complémentaire (pas redondant, pas décorrélé)."
    )
    rows.append("")
    ...
    rows.append(
        "Un trade peut être rejeté par AU PLUS un filtre (l'ordre F1 → F2 → F3 est respecté)."
    )
    rows.append("")
    ...
    # Synthèse globale
    all_reject = (
        df_v5["v5_reject_reason"].fillna("NO_REJECT").value_counts()
        if "v5_reject_reason" in df_v5.columns
        else pd.Series()
    )
    rows.append("")
