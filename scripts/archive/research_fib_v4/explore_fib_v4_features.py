...
def main():
    ...
    if not baseline_csv.exists():
        print("Baseline introuvable. Exécute d'abord scripts/baseline_fib_v4.py")
        sys.exit(1)
    ...
    for _, r in viable.iterrows():
        print(
            f"    • {r['ticker']} {r['tf']} fib={r['fib_level']}  (PF={r['pf']:.2f}, n={int(r['n_filled'])})"
        )
    ...
    for _, r in viable.iterrows():
        ...
        if len(trades):
            ...
            print(
                f"    [{r['ticker']:>5} {r['tf']:>3} {r['fib_level']:.3f}] "
