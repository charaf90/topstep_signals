...
def main():
    ...
    bars = client.get_bars(contract_id, start, end, unit=2, unit_number=1, limit=10, live=False)
    if not bars:
    ...
    print("   SL bracket : -10 ticks (= -2.5 pts = ~$12.50 max loss)")
    print("   TP bracket : +10 ticks (= +2.5 pts = ~$12.50 max gain)")
