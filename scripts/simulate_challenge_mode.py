...
from core.adaptive_sizing import adaptive_risk_usd, trading_days_until  # noqa: E402
...
def load_bootstrap_pool(output_dir: Path) -> Dict[str, List[TradeOutcome]]:
    ...
    files = list(output_dir.glob("backtest_*_opr.csv")) + \
            list(output_dir.glob("backtest_*_fib.csv"))
    for f in files:
...
def simulate_one_challenge(
    ...
    for day_idx in range(n_days):
        ...
        trades_today = (
            [("OPR", t) for t in rng.choice(pool["OPR"], size=n_opr, replace=True)]
        ...
        for strat, trade in trades_today:
            if state.busted or state.target_hit:
                break
            # Sizing
            signal = {"strategy": strat, "sl_dist": 20.0}  # sl_dist factice (utilisé seulement par n_ct dans live_runner)
...
def run_simulation(
    ...
    print(f"Pool OPR : {len(pool['OPR'])} trades | "
          f"Pool FIB : {len(pool['FIB'])} trades", file=sys.stderr)
    if len(pool["OPR"]) < 30 or len(pool["FIB"]) < 5:
        print("⚠️ Pool trop petit — résultats peu fiables", file=sys.stderr)

    states = [simulate_one_challenge(pool, rng, sizing_mode=sizing_mode)
              for _ in range(n)]
    ...
    return {
        ...
        "median_days_to_target": (
            float(np.median(target_days)) if len(target_days) > 0 else -1
        ),
    }
...
def print_result(r: Dict):
    ...
    print(f"  EV PnL p25/p50/p75: ${r['EV_pnl_p25']:+.0f} / ${r['EV_pnl_p50']:+.0f} / ${r['EV_pnl_p75']:+.0f}")
...
def main():
    ...
    print(f"  Δ EV net         : ${(r_adapt['EV_net_after_sub']-r_static['EV_net_after_sub']):+8.0f}")
    ...
    for name, ok in [("P(target) ≥ 35%", ok_target),
                     ("P(bust) ≤ 15%", ok_bust),
                     ("EV net > 0", ok_ev),
