...
from config import (
    ...
    KIJUN_PB_TICKERS,
    KIJUN_PB_TIMEZONE,
    KIJUN_PB_SESSION_START, KIJUN_PB_SESSION_END,
    KIJUN_PB_TENKAN_PERIOD, KIJUN_PB_KIJUN_PERIOD,
    KIJUN_PB_SENKOU_B_PERIOD, KIJUN_PB_KUMO_SHIFT,
    KIJUN_PB_RSI_PERIOD, KIJUN_PB_STOCH_PERIOD,
    KIJUN_PB_STOCH_K, KIJUN_PB_STOCH_D,
    KIJUN_PB_STOCHRSI_OVERSOLD, KIJUN_PB_STOCHRSI_OVERBOUGHT,
...
from core.explore_chart import (
    compute_ichimoku, compute_stochrsi, compute_atr,
)
...
def run_backtest(
    ...
    for i in range(warmup, n):
        ...
        for col in ("kijun", "senkou_a", "senkou_b", "atr", "stoch_k", "stoch_d"):
            ...
            kijun_slope = float(prev["kijun"]) - float(df.iloc[i - 1 - KIJUN_PB_SLOPE_LOOKBACK]["kijun"])
            ...
            k1 = float(prev["stoch_k"]);  d1 = float(prev["stoch_d"])
            k2 = float(prev2["stoch_k"]); d2 = float(prev2["stoch_d"])
            ...
            if (cond_long_cloud and cond_long_slope and cond_long_pb
                    and cond_long_above and cond_long_cross):
                ...
                signal = {"direction": "long", "entry": entry, "sl": sl_px,
                          "tp": tp_px, "setup_low": low_pullback,
            ...
            if signal is None:
                ...
                if (cond_short_cloud and cond_short_slope and cond_short_pb
                        and cond_short_below and cond_short_cross):
                    entry = float(prev["close"])
                    ...
                    sl_px = sl_base
                    tp_px = entry - tp_atr_mult * atr_val
                    signal = {"direction": "short", "entry": entry, "sl": sl_px,
                              "tp": tp_px, "setup_low": low_pullback,
                              "setup_high": high_pullback}
            ...
            trades.append({
                ...
                "atr_pct":      round(float(prev["atr_pct"]), 2) if pd.notna(prev["atr_pct"]) else None,
