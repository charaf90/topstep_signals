...
def _compute_adx(df: pd.DataFrame, period: int = 14) -> pd.Series:
    """ADX pour classification trending/ranging (utilisé en stress tests PHASE 5)."""
    high, low, close = df["high"], df["low"], df["close"]
    up_move   = high.diff()
    down_move = -low.diff()
...
def _compute_emas(df: pd.DataFrame, fast: int, slow: int) -> Tuple[pd.Series, pd.Series]:
    """EMA rapide et lente pour le filtre tendance."""
    return (
...
def _compute_profile(
    ...
    """
    Construit un volume profile approximé à partir des barres M15 d'une session.

    Hypothèse pragmatique : sur chaque barre, on distribue le volume uniformément
    sur les buckets traversés par le range [low, high]. C'est plus fidèle que
    "tout au close" et reste sans look-ahead.

    Retourne un dict :
        poc : prix du bucket de volume maximal
        vah : haut du Value Area (extrême haut)
        val : bas du Value Area (extrême bas)
        hvn_levels : liste de prix des HVN (volume ≥ hvn_mult × moyenne)
        lvn_levels : liste de prix des LVN (volume ≤ lvn_mult × moyenne)
        profile_high / profile_low : extrêmes de la session
    """
    ...
    # Buckets : on borne par sécurité (range / bucket_size capé à 500 buckets)
    n_buckets = int(np.ceil((p_high - p_low) / bucket_size)) + 1
    if n_buckets < 3:
        return None
    if n_buckets > 500:
        # range anormalement grand → on adapte la taille de bucket
    ...
    for _, bar in day_bars.iterrows():
        ...
        if b_high <= b_low:
            continue
        # Buckets traversés par cette barre
    ...
    # POC = bucket de volume max
    poc_idx = int(np.argmax(vols))
    poc     = float(centers[poc_idx])

    # Value Area = extension symétrique autour du POC jusqu'à atteindre VA_PCT
    ...
    # HVN / LVN par seuil multiplicatif vs moyenne
    mean_v = vols[vols > 0].mean() if (vols > 0).any() else 0.0
    if mean_v <= 0:
        hvn_levels, lvn_levels = [], []
    else:
        hvn_mask = vols >= hvn_mult * mean_v
        lvn_mask = (vols > 0) & (vols <= lvn_mult * mean_v)
        hvn_levels = [float(centers[i]) for i in np.where(hvn_mask)[0]]
        lvn_levels = [float(centers[i]) for i in np.where(lvn_mask)[0]]
...
def _build_previous_day_profile(
    ...
    """
    Construit le profil de la session cash NY veille (9h30-16h00) en se basant
    strictement sur des barres timestampées AVANT day_ny 00:00 NY.
    Sans look-ahead.
    """
    ...
    # On cherche la dernière session NY cash AVANT day_ny.
    # Stratégie : reculer jusqu'à 5 jours pour trouver une session valide
    for back in range(1, 6):
        ...
        # Masque sur barres dont l'OUVERTURE est dans la session veille
        mask = (idx_ny >= start) & (idx_ny < end)
        if mask.sum() < 4:
            continue
        day_bars = df_15m.iloc[np.asarray(mask)]
        prof = _compute_profile(
            day_bars, tick_size, bucket_ticks,
            value_area_pct, hvn_mult, lvn_mult,
        )
        if prof is not None:
...
# ══════════════════════════════════════════════════════════════════════════════
# Setups — détection sur la bougie courante avec données strictement passées
# ══════════════════════════════════════════════════════════════════════════════
...
def _detect_setup_open_outside(
    ...
    """
    Setup 1 — open hors Value Area veille.
    Déclenchement uniquement sur la PREMIÈRE barre où l'open NY est connu
    (9h30 NY).

    Mode CONTINUATION (fade=False) : direction = sens du gap
    Mode FADE (fade=True) : direction = inverse du gap (retour vers VA)

    Filtre : taille du gap ≥ min_gap_atr × atr (sinon = pas de vraie ouverture
    hors profil).
    """
    ...
    # Filtre taille gap
    if open_px > vah:
        gap_size = open_px - vah
        gap_dir  = "above"
    elif open_px < val:
        gap_size = val - open_px
        gap_dir  = "below"
    else:
        return None
    ...
    if fade:
        # FADE : gap haut → short vers VA, gap bas → long vers VA
        if gap_dir == "above":
            direction = "short"
            entry = open_px
            sl    = open_px + max(buf, atr * sl_atr_mult)
            # TP : retour vers VAH (premier objectif) ou POC si gap large
            tp_geom = vah if (open_px - vah) >= 0.5 * atr else poc
            tp_atr  = open_px - atr * tp_atr_mult
            # Prend le plus proche (TP réaliste = retour vers VA, pas POC)
            tp = max(tp_geom, tp_atr)  # 'max' car short : plus proche du open
            return {
                "setup": "OPEN_OUTSIDE",
                "direction": direction,
                "entry": entry,
            ...
            entry = open_px
            sl    = open_px - max(buf, atr * sl_atr_mult)
            tp_geom = val if (val - open_px) >= 0.5 * atr else poc
            tp_atr  = open_px + atr * tp_atr_mult
            tp = min(tp_geom, tp_atr)
            return {
                "setup": "OPEN_OUTSIDE",
                "direction": direction,
                "entry": entry,
    ...
    # CONTINUATION (mode original)
    if gap_dir == "above":
        direction = "long"
        sl_geom = val - buf
        sl_atr  = open_px - atr * sl_atr_mult
        sl = max(sl_geom, sl_atr)
        tp = open_px + atr * tp_atr_mult
        return {
            "setup": "OPEN_OUTSIDE",
            "direction": direction,
            "entry": open_px,
...
def _detect_setup_breakout_retest(
    ...
    """
    Setup 2 — cassure de VAH/VAL avec volume confirmé, retest pour entrée limit.

    Condition LONG :
      - close[prev] > vah (cassure haute confirmée sur barre précédente)
      - volume[prev] > vol_threshold × moyenne
      - EMA_fast > EMA_slow (filtre tendance)
      - low[bar] ≤ vah ≤ high[bar] (retest sur barre courante)

    Entrée limit @ VAH (support retourné), SL sous VAH, TP vers POC ou ATR.
    """
    ...
    # LONG : cassure VAH veille + retest
    if (float(prev["close"]) > vah
        and ema_fast_val > ema_slow_val
        and float(bar["low"]) <= vah <= float(bar["high"])):
        entry = vah
        sl    = vah - max(buf, atr * sl_atr_mult)
        # TP : prochain HVN au-dessus ou ATR mult
        tp_geom = poc if poc > vah else (vah + atr * tp_atr_mult)
        tp_atr  = entry + atr * tp_atr_mult
        tp = max(tp_geom, tp_atr) if poc > vah else tp_atr
        return {
    ...
    # SHORT : cassure VAL veille + retest
    if (float(prev["close"]) < val
        and ema_fast_val < ema_slow_val
        and float(bar["low"]) <= val <= float(bar["high"])):
        entry = val
        sl    = val + max(buf, atr * sl_atr_mult)
        tp_geom = poc if poc < val else (val - atr * tp_atr_mult)
        tp_atr  = entry - atr * tp_atr_mult
        tp = min(tp_geom, tp_atr) if poc < val else tp_atr
        return {
            "setup": "BREAKOUT_RETEST",
            "direction": "short",
            "entry": entry,
...
def _detect_setup_hvn_rebound(
    ...
    """
    Setup 3 — bougie de rejet sur un HVN, entrée à l'extrême du HVN.

    Condition LONG :
      - low[bar] ≤ HVN ≤ open et close de bar (mèche basse sur HVN)
      - mèche basse ≥ 50% du corps (rejet)
      - volume[bar] > vol_threshold × moyenne
      - EMA_fast > EMA_slow
      - close > open (bougie verte de rejet)

    Entrée limit @ bas du HVN (relativement au touch), SL sous HVN + buffer,
    TP vers POC.
    """
    ...
    if avg_vol <= 0 or float(bar["volume"]) <= vol_threshold * avg_vol:
        return None
    # Filtre ADX : exclut les régimes ranging mort
    ...
    # LONG : bougie verte avec longue mèche basse touchant un HVN
    if c > o and ema_fast_val > ema_slow_val:
        lower_wick = min(o, c) - l
        if lower_wick >= 0.5 * body:
            # HVN touché par la mèche basse, sous le corps
            candidates = [hvn for hvn in profile["hvn_levels"]
                          if l <= hvn <= min(o, c) and hvn < poc]
            if candidates:
                hvn = max(candidates)  # le plus haut HVN touché par la mèche
                entry = hvn
                sl = hvn - max(buf, atr * sl_atr_mult * 0.7)  # SL plus serré sur rebound
                tp = poc
                # Filtre RR minimum
                sl_d = entry - sl
                tp_d = tp - entry
                ...
                return {
                    ...
                    "entry": entry,
                    "sl": sl, "tp": tp,
                    "setup_low": l, "setup_high": h,
                }

    # SHORT : bougie rouge avec longue mèche haute touchant un HVN
    if c < o and ema_fast_val < ema_slow_val:
        upper_wick = h - max(o, c)
        if upper_wick >= 0.5 * body:
            candidates = [hvn for hvn in profile["hvn_levels"]
                          if max(o, c) <= hvn <= h and hvn > poc]
            if candidates:
                hvn = min(candidates)
                entry = hvn
