...
def _baseline_v4(df, ticker, fib_level=0.382):
    ...
    return run_fib_v4_backtest(
        df, ticker, fib_level=fib_level,
        pivot_break_buffer_atr=1e9, wick_max_atr=1e9, skip_macro=False,
    )


def _synthetic_df(n_bars: int = 2000, seed: int = 42,
                   ticker_pivots: bool = True) -> pd.DataFrame:
...
def test_fib_v4_no_more_trades_than_baseline(df_real: pd.DataFrame):
    ...
    trades_v4 = run_fib_v4_backtest(df_real, "MES1", fib_level=0.382,
                                     pivot_break_buffer_atr=0.0, wick_max_atr=0.05)
    filled_base = trades_base[trades_base["result"].isin(["TP", "SL", "TE"])]
    filled_v4 = trades_v4[trades_v4["result"].isin(["TP", "SL", "TE"])]
    assert len(filled_v4) <= len(filled_base), \
...
def test_fib_v4_filters_reduce_to_zero_at_extreme(df_real: pd.DataFrame):
    ...
    trades_extreme = run_fib_v4_backtest(df_real, "MES1", fib_level=0.382,
                                           pivot_break_buffer_atr=0.0,
                                           wick_max_atr=-1.0)
...
def test_fib_v4_filters_inactive_idempotent(df_real: pd.DataFrame):
    ...
    trades_a = run_fib_v4_backtest(df_real, "MES1", fib_level=0.382,
                                    pivot_break_buffer_atr=1e9, wick_max_atr=1e9)
    trades_b = run_fib_v4_backtest(df_real, "MES1", fib_level=0.382,
                                    pivot_break_buffer_atr=1e9, wick_max_atr=1e9)
    filled_a = trades_a[trades_a["result"].isin(["TP", "SL", "TE"])]
    filled_b = trades_b[trades_b["result"].isin(["TP", "SL", "TE"])]
    assert len(filled_a) == len(filled_b), \
        f"Idempotence : 2 runs identiques attendus, got {len(filled_a)} vs {len(filled_b)}"
    pnl_a = filled_a["pnl"].sum()
    pnl_b = filled_b["pnl"].sum()
    assert abs(pnl_a - pnl_b) < 0.01, \
...
def test_fib_v4_pivot_break_invariant(df_real: pd.DataFrame):
    ...
    trades = run_fib_v4_backtest(df_real, "MES1", fib_level=0.382,
                                  pivot_break_buffer_atr=buffer, wick_max_atr=1e9)
...
def test_fib_v4_temporal_consistency(df_real: pd.DataFrame):
    ...
    for _, t in filled.iterrows():
        ...
        assert pt <= ft <= et, \
            f"Inversion temporelle : pending={pt}, fill={ft}, exit={et}"
...
def test_fib_v4_runs_on_synthetic(df_synth: pd.DataFrame):
    ...
    trades = run_fib_v4_backtest(df_synth, "MES1", fib_level=0.382,
                                  pivot_break_buffer_atr=0.0, wick_max_atr=0.5)
    assert isinstance(trades, pd.DataFrame)
    if len(trades) > 0:
        required = ["entry", "sl", "tp", "pnl", "result", "pivot_break_atr",
                    "wick_through_atr"]
        for col in required:
