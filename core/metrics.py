...
def compute_stats(df_trades: pd.DataFrame) -> dict:
    ...
    empty = {"n": 0, "wr": 0.0, "pf": 0.0, "pnl": 0.0, "dd": 0.0,
             "sharpe": 0.0, "avg_win": 0.0, "avg_loss": 0.0, "n_signals": 0}
...
def compute_topstep(df_trades: pd.DataFrame, n_bootstrap: int = 1000) -> dict:
    ...
    if df_trades is None or len(df_trades) == 0:
        return {"n_days": 0, "bootstrap_pass_rate": 0.0,
                "violates_daily": False, "violates_trailing": False}

    filled = df_trades[df_trades["result"] != "NOT_FILLED"].copy() \
             if "result" in df_trades.columns else df_trades.copy()
    if len(filled) == 0:
        return {"n_days": 0, "bootstrap_pass_rate": 0.0,
                "violates_daily": False, "violates_trailing": False}
...
def print_stats_report(stats: dict, ticker: str, strategy_id: str,
...
def print_topstep_report(ts: dict, ticker: str):
    ...
    print(f"  Jours         : {ts['n_days']} ({ts['n_winning_days']} win, "
          f"ratio {ts['winning_ratio']*100:.0f}%)")
    print(f"  Perte jour max: ${ts['max_daily_loss']:+,.0f}   "
          f"{fd} (limite -${TOPSTEP_DAILY_LOSS_MAX})")
    print(f"  Trailing DD   : ${ts['trailing_dd']:+,.0f}   "
          f"{ft} (limite -${TOPSTEP_TRAILING_DD})")
...
def print_portfolio_report(results_by_strategy: dict, n_bootstrap: int = 1000):
    ...
    for strat_id, results in results_by_strategy.items():
        all_trades = [r["df_trades"] for r in results
                      if r.get("df_trades") is not None and len(r["df_trades"]) > 0]
        if not all_trades:
            continue
        combined = pd.concat(all_trades, ignore_index=True)
        filled   = combined[combined["result"] != "NOT_FILLED"] \
                   if "result" in combined.columns else combined
        ...
        print(f"  Jours         : {len(days)} ({n_win} win, "
              f"ratio {n_win/len(days)*100:.0f}%)")
        ...
        print(f"  Bootstrap     : {bs*100:.1f}%  "
              f"(cible ≥ 80%, target $+{TOPSTEP_PROFIT_TARGET})")
...
def print_verdict_report(strategy_id: str,
                         is_stats: dict, oos_stats: dict,
                         oos_topstep: dict,
                         is_period: str = "", oos_period: str = ""):
    ...
    print(f"  {'═'*60}")
    if is_period:
        print(f"  IS  ({is_period}) : "
              f"PF={is_stats['pf']:.2f}  P&L=${is_stats['pnl']:+,.0f}  "
              f"n={is_stats['n']}")
    if oos_period:
        print(f"  OOS ({oos_period}) : "
    ...
    if v_label == "PRODUCTION":
        ...
        if oos_topstep.get("bootstrap_pass_rate", 0) < VERDICT_VEILLE["bootstrap"]:
            reasons.append(f"BS={oos_topstep.get('bootstrap_pass_rate',0)*100:.0f}% < "
